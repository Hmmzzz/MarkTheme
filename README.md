...
当前版本为 `v0.2.3`。两种越狱环境使用不同软件包，请勿混装。

`v0.2.3` 修复全局图标 overlay 在高频显示 Getter 上重复解析与合成所造成的发热和能耗问题。
overlay 与自定义 mask 各自只读取一个全局源资源，次级尺寸改为按需派生；桌面显示路径使用资源
代际与 view 结果缓存，并让相同底层 raster 跨 `UIImage` 包装复用合成像素。缓存扩容仍受原有
16 MB 成本上限约束，内存压力会释放派生结果，主题关闭与切换时继续准确恢复原始图像。
