...
当前版本为 `v0.2.5`。两种越狱环境使用不同软件包，请勿混装。

`v0.2.5` 继续收敛显示与主题切换路径的 CPU、锁和对象分配开销。遮罩启用状态改为原子标量
快照，静态图标预热直接按 Generation 索引筛选实际存在资源的 App，并跳过无主题资源批次的
解析与主线程往返；重复的刷新登记、状态栏与 UI 资源解析、已取消预览任务也会提前终止。
同时移除图标生产热路径中未被报告消费的诊断写入，保留原有完整性校验、缓存上限、回滚与
系统原生 fallback 行为。
